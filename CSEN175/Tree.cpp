 ...
 *
 * Description:	Initialize a number, which has the given type.
 */

Number::Number(long value, int specifier)
    : Expression(Type(specifier)), _value(value)
{
}
    ...
    : Expression(Type(INT))
{
    ...
    errno = 0;
    _value = strtol(value.c_str(), &ptr, 0);

    if (errno != 0)
	_type = Type();
    else if (_value != (int) _value)
	_type = Type(LONG);
}
...
bool Expression::isNumber(long &value) const
{
    return false;
}
...
bool Number::isNumber(long &value) const
{
    value = _value;
    return true;
}
