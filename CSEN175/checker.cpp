...
static const Type error, integer(INT), longint(LONG);
...
static Expression *scale(Expression *expr, unsigned long size)
{
    long value;
 ...
 * Function:	checkCall
 *
 * Description:	Check a function call expression: ID (ARGS).  All arguments
 *		first undergo type decay and afterward must be scalar
 *		types.  If the parameters of the function have been
 *		specified, each actual must be compatible with its
...
Expression *checkCall(Symbol *id, Expressions &args)
{
	...
	for (auto &arg : args)
	    if (arg->type() != error && !decay(arg).isScalar()) {
 ...
 *		operand undergoes the usual conversions, and then must have
 *		a numeric type, and result has the same type.
 */
