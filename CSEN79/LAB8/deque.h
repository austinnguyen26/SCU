...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                {
                    // MY WORK...
                    int *index = source.block_pointers[bp_array_index] + block_item_index;
                    // Checking to see if index is at the back
                    if (index == source.back_ptr){
                        back_ptr = block_pointers[bp_array_index] + block_item_index;
                    }
                    //Check to see if index is at the front of source
                    if (index == source.front_ptr){
                        front_ptr = block_pointers[bp_array_index] + block_item_index;
                    }
                    *(block_pointers[bp_array_index] + block_item_index) = *(index);
                }
            }
        }
    }
    ...
    deque<Item>::~deque () {
        ...
        // Freeing memory
        delete[] block_pointers;
        // Set all relevant pointers = NULL to prevent memory leaks
    ...
    void deque<Item>::clear () {
        ...
        // Clear the data blocks
        // MY WORK...
        // Iterate through data blocks, setting pointers to null and freeing from memory
        ...
        // Setting relevant pointers = NULL
        first_bp = last_bp = NULL;
        front_ptr = back_ptr = NULL;
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // MY WORK...
            // allocate memory, assign front and back ptrs respectively to first and last bp
            // then assign back_ptr as entry
        ...
        {
            // MY WORK...
            // front_ptr points to empty slot before entry
            *--front_ptr = entry;
        }
        ...
        {
            // MY WORK...
            // Decremement first_bp, then allocate memory for it
            // assign front_ptr as entry based on first_bp and block_size
            // *--first_bp = new value_type[block_size];
            --first_bp;
            *first_bp = new value_type[block_size];
        ...
        {
            // MY WORK...
            // call reserve to expand the amount of space in the deque
            reserve();
            // *--first_bp = new value_type[block_size];
            --first_bp;
            *first_bp = new value_type[block_size];
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            *first_bp = new value_type[block_size];
            ...
            // MY WORK...
            // point back_ptr to last_bp, and assign it as entry
            back_ptr = *last_bp;
            *back_ptr = entry;
            front_ptr = *first_bp;
        ...
        {
            // MY WORK...
            // increment back_ptr and assign it as entry
            *++back_ptr = entry;
        ...
        {
            // MY WORK...
            // increment last_bp, allocate memory for it
            // point back_ptr to last_bp and assign it as entry point
        ...
        {
            // MY WORK...
            // call reserve to expand deque and follow same logic as above
            reserve();
            // *++last_bp = new value_type[block_size];
            ++last_bp;
            *last_bp = new value_type[block_size];
    ...
    void deque<Item>::pop_front()
    {
        ...
        {
            // MY WORK..
            // calling clear function to delete items within deque 
            clear();
        ...
        {
            // MY WORK...
            // freeing memory using delete[] and then setting appropritate pointers = NULL
            delete[] *first_bp;
            *first_bp = NULL;
            ++first_bp;
            front_ptr = *first_bp;
    ...
    void deque<Item>::pop_back()
    {
        ...
        {
            // MY WORK...
            // delete the back pointer (pop_back) and set last_bp = NULL
            // reassign back_ptr so that you don't lose it or have memory leaks
            delete[] back_ptr;
            *last_bp = NULL;
            --last_bp;
